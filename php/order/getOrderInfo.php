...
if (!isset($_COOKIE['username'])) {
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') { // Handle the form.
		...
		$orderscontent .= ']';
		$orderfile = fopen("../../json/order/order.json", "w");
		fwrite($orderfile, $orderscontent);
		fclose($orderfile);

		// to write orderdetail
		$q = "SELECT oc_id, order_id, product_id, quantity, price, color, size, cmt_status, user_id, selected, product_name, img_name FROM order_content LEFT JOIN products USING(product_id) LEFT JOIN imges USING(product_id) WHERE cover='Y' AND cmt_status='Y' AND user_id={$_POST['user_id']}";
		$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));	
		$orderscontent = '[';
		while($eachorder = mysqli_fetch_array($r, MYSQLI_ASSOC)) {
			$orderscontent .= '{"user_id":' . $eachorder['user_id'] . 
		...
		$orderfile = fopen("../../json/order/orderdetail.json", "w");
		fwrite($orderfile, $orderscontent);
		fclose($orderfile);
	}
}
